...
Today this gap is filled by:
- `ubml validate --suggest` (Plan 08 C) — flat list of suggestions, no priority
- `ubml doctor` (Plan 10 B) — workspace-level health, not element-level
...
### D.1: YAML Schema Validation (IDE Tooltips)

JSON Schema doesn't natively support conditional refinement questions. However, the `x-ubml-questions` annotations are preserved in the published schema and can be consumed by:

1. **Schema-aware editors** — Red Hat YAML extension reads `description` fields. We enhance property descriptions to include refinement hints:

```yaml
# Enhanced descriptions in schema (generated by build)
RACI:
  description: |
    RACI matrix for this step.
    
    💡 Refinement: Who is responsible for performing this step?
    Assign RACI to enable swim lane diagrams and responsibility views.
```

**Implementation**: The build pipeline enhances property descriptions with refinement hints extracted from `x-ubml-questions`. This is a no-op for strict validation but enriches IDE tooltips.

**Files modified:**
- `scripts/generate/bundle-schemas.ts` — Inject refinement hints into property descriptions during bundling

### D.2: ESLint Rules (Inline Warnings)
...
### Phase 5: IDE & Schema Enhancement

- [ ] Enhance `scripts/generate/bundle-schemas.ts` to inject refinement hints into property descriptions
- [ ] Verify enriched tooltips appear in VS Code with Red Hat YAML extension
